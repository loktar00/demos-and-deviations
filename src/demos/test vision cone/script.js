...
function shineLight(light){
    ...
    for(curAngle; curAngle < light.angle + (light.angleSpread/2); curAngle += (addTo * (180/Math.PI))*1){
        dynLen = light.radius;
...
function draw(){
    ...
    for(var i = 0; i < lights.length; i++){
        ...
        lights[i].angle += 1;//Math.sin(angle)*2;
        shineLight(lights[i]);
    }
...
lights.push(new light(new vector(256,256), 300, 60, 'rgba(255,255,255,0.1)'));
