...
(function () {
    ...
    {   
		...
		// make a bigger image object with the background 4x4 so we can scroll it easier. Need to look at performance cost for this later
		/*var tempCanvas = document.createElement("canvas"),
			tempCtx = tempCanvas.getContext("2d"),
			source = this.resource.source;
		
		tempCanvas.width = this.endX * 2;
		tempCanvas.height = this.endY * 2;
		
		for(var x = 0; x < 2; x++){
			for(var y = 0; y < 2; y++){
				tempCtx.drawImage(source,0,0,this.endX,this.endY,this.endX*x, this.endY*y, this.endX, this.endY);	
			}
		}
		
		this.resource.source = tempCanvas;*/
		//Game.addEntity(this,false,this.state);
    }
	...
	Background.prototype = {
		...
		scroll : function(speedX, speedY){	
			this.scrollX += speedX/10;
			this.scrollY += speedY/10;
		...
		render : function(_context){
			...
			if(this.startY + this.scrollY > this.endY-this.height || this.startX + this.scrollX > this.endX-this.width){
				...
				//_context.drawImage(source,this.startX+this.scrollX,this.startY+this.scrollY,this.width,this.endY -(this.startY+this.scrollY), this.pos.x, this.pos.y, this.width, this.endY-(this.startY+this.scrollY));	
				//_context.drawImage(source,this.startX,this.startY,this.width,this.height,this.pos.x, this.pos.y+this.endY-(this.startY+this.scrollY), this.width, this.height);	
			}else{
