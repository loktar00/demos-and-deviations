...
var addRing = function (xx, yy) {
    ...
    points[xx * size + yy] = new setPoint(x, y, z, [~~ (Math.random() * 200), ~~ (Math.random() * 200), ~~ (Math.random() * 200)]);
}
    ...
    for (j = size - 1; j > -1; j--) {
        for (i = size - 1; i > -1; i--) {
            ...
            if (j < size - 1 && i < size - 1 && pz > -250) {
                ctx.lineTo(~~points[((j + 1) * size) + i].x, ~~points[((j + 1) * size) + i].y);
                ctx.lineTo(~~points[(j + 1) * size + (i + 1)].x, ~~points[(j + 1) * size + (i + 1)].y);
                ctx.lineTo(~~points[(j * size) + (i + 1)].x, ~~points[(j * size) + (i + 1)].y);
            }
